...
MNT = [
    (0, 'VEDA', 0)  # Format: (index, macro_name, address in MDT)
]

MDT = [
    (0, "VEDA &arg1, &arg2"),  # Macro definition header
...
ALA = [
    {0: 'arg1', 1: 'arg2'}  # Argument list for macro 'VEDA'
]
...
source_program_pass2 = [
    ...
    "A2, &arg2",
    "MEND", 
    "VEDA data1, data2",  # Macro invocation with arguments
...
def expand_macros(source_program):
    ...
    for line in source_program:
        ...
        # Step 3: Check if the line contains a macro invocation (e.g., "VEDA data1, data2")
        for mnt_entry in MNT:
            macro_name = mnt_entry[1]  # Extract macro name (e.g., 'VEDA')
...
print("| Macro Index | Argument Name | Value   |")
print("|-------------|---------------|---------|")
for index, args in ALA[0].items():
    print(f"| {index:<11} | arg{index+1:<13} | {args:<7} |")
